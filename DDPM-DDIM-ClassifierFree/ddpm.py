...
epochs = 201
batch_size = 64
image_size = 28 # cifar10  = 32 or mnist, fmnist 28
timesteps = 300 # ddim_steps = 50
in_channels = 1 # cifar = 3, mnist, fmnist = 1
model_channel = 128
ds = 'mnist_step300_ep201'# cifar10 or fmnist
exp_name = './ddpm_'+ds # cifar10 or fmnist
...
class GaussianDiffusion:
    ...
    def p_sample(self, model, x_t: torch.FloatTensor, t: torch.LongTensor):
        ...
        model_mean, posterior_variance, model_log_variance = self.p_mean_variance(model, x_t, t)
...
if train == True: # training ddpm model
    model = UNetModel(io_channels=in_channels,model_channels=model_channel).to(device)
    ...
    for epoch in range(epochs):
        ...
        if (epoch == 1) or (epoch % 10 == 0):
            with torch.no_grad():
                ...
                torchvision.utils.save_image(imgs, './sample_noise_train=%s.png'%train, nrow = n_cols+1)
    ...
    model = UNetModel(io_channels=in_channels, model_channels=128).to(device) #加载预训练模型
    ...
    with torch.no_grad():
        ...
        torchvision.utils.save_image((generated_images[-1]+ 1)/2, './samples_train=%s_ddim=%s.png'%(train,ddim), nrow = 8)
