...
os.environ['CUDA_VISIBLE_DEVICES'] = '0' # 设置GPU id 默认为0
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

epochs = 501
batch_size = 64
image_size = 32 # cifar10  = 32 or fmnist 28
timesteps = 1000 # ddim_steps = 50
in_channels = 3
ds = 'cifar_step1000_ep501_v2=bigModel'# cifar10 or fmnist
...
class UNetModel(nn.Module):
    def __init__(self, io_channels=3, model_channels=128, class_num = None):
        ...
        self.down_block5 = nn.Conv2d(model_channels*2, model_channels*4, kernel_size=3, padding=1)
        self.down_block6  =  Downsample(model_channels*4)

        self.middle_block = ResidualBlock(model_channels*4, model_channels*4) # middle block
        self.noise_embedding = nn.Linear(model_channels, model_channels*4) # noise block
        if class_free != None:
            self.class_emb = nn.Embedding(class_num, model_channels*4)
        self.middle_block = ResidualBlock(model_channels*4, model_channels*4) # middle block
        self.noise_embedding = nn.Linear(model_channels, model_channels*4) # noise block
        if class_free != None:
            self.class_emb = nn.Embedding(class_num, model_channels*4)

        self.up_block1 = Upsample(model_channels*4) #  # up blocks
        self.up_block2 =  nn.Conv2d(model_channels*4, model_channels*2, kernel_size=3, padding=1)
        self.up_block3 = Upsample(model_channels*2) #  # up blocks
        self.up_block4 =  nn.Conv2d(model_channels*2, model_channels, kernel_size=3, padding=1)
        self.up_block5 = Upsample(model_channels)
        self.up_block6 =  nn.Conv2d(model_channels, io_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x, t, label=None):
        ...
        x5 = F.relu(F.group_norm(self.down_block5(x4), num_groups=32))
        x6 = F.relu(F.group_norm(self.down_block6(x5), num_groups=32))

        middle1 = self.middle_block(x6) # Middle block
        noise_t1 = F.relu(self.noise_embedding(timestep_embedding(t,self.model_channels)))
        if label != None:
            c_emb1 = F.relu(self.class_emb(label))
            middle1 = middle1 + noise_t1[:, :, None, None] + c_emb1[:, :, None, None]
        else:
            middle1 = middle1 + noise_t1[:, :, None, None]

        middle2 = self.middle_block(middle1) # Middle block
        noise_t2 = F.relu(self.noise_embedding(timestep_embedding(t,self.model_channels)))
        if label != None:
            c_emb2 = F.relu(self.class_emb(label))
            middle2 = middle1 + noise_t2[:, :, None, None] + c_emb2[:, :, None, None]
        else:
            middle2 = middle1 + noise_t2[:, :, None, None]

        x7 = F.relu(F.group_norm(self.up_block1(middle2 + x6), num_groups=32)) # Decode-Upsampling
        x8 = F.relu(F.group_norm(self.up_block2(x7 + x5), num_groups=32)) # 
        x9 = F.relu(F.group_norm(self.up_block3(x8 + x4), num_groups=32)) #  
        x10 = F.relu(F.group_norm(self.up_block4(x9 + x3), num_groups=32)) # 
        x11 = F.relu(F.group_norm(self.up_block5(x10 + x2), num_groups=32)) #  
        out = self.up_block6(x11 + x1) #  
...
class GaussianDiffusion:
    def __init__(self, timesteps=1000,):
        ...
        self.sqrt_alphas_cumprod = torch.sqrt(self.alphas_cumprod) # 从1到0的数, 递减
        self.sqrt_one_minus_alphas_cumprod = torch.sqrt(1.0 - self.alphas_cumprod) # # 从0到1数, 递增
